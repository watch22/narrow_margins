#load packages ----
pacman::p_load(tidyverse, StatsBombR, extrafont, ggupset, tibbletime, ggtext, ggrepel, glue, patchwork, cowplot, gtable, grid, magick, here, ggsoccer, janitor, rvest, ggimage, zoo, ggforce, knitr, kableExtra)
...
season_clean <- season_df %>% 
  ...
  mutate(competition = case_when(competition == "bundesliga" ~ "Bundesliga",
         ...
         season = case_when(season == 2017 ~ "2017/18",
                            season == 2018 ~ "2018/19",
                            season == 2019 ~ "2019/20",
                            season == 2020 ~ "2020/21",
                            season == 2021 ~ "2021/22")) %>% 
...
pma_by_team_season <- xga_wt_rlnt_all_fix %>% 
  ...
  mutate(min_ahead_per_game = round(min_ahead/M,1),
         norm_xGpMA = round(total_xG_ahead/min_ahead_per_game,4),
         norm_xGpMA_wt = round(weighted_xG_ahead/min_ahead_per_game,4),
         norm_percent_diff_for = (signif((norm_xGpMA_wt-norm_xGpMA)/norm_xGpMA,4)*100),
         norm_xGApMA = round(total_xGA_ahead/min_ahead_per_game,4),
         norm_xGApMA_wt = round(weighted_xGA_ahead/min_ahead_per_game,4),
         norm_percent_diff_against = (signif((norm_xGApMA_wt-norm_xGApMA)/norm_xGApMA,4)*100),
         one_ahead_per_game = round(one_ahead/M,1),
         norm_xG_one_ahead = round(total_xG_one_ahead/one_ahead_per_game,4),
         norm_xGA_one_ahead = round(total_xGA_one_ahead/one_ahead_per_game,4)) %>% 
  ungroup()
...
pma_xGD <- pma_xG_xGA %>% 
  ...
  select(1:6,min_ahead, one_ahead, contains('xGD'))
...
pma_by_team <- xga_wt_rlnt_all_fix %>% 
  group_by(team) %>% 
  summarise(across(where(is.numeric), ~ sum(.x))) %>% 
  mutate(xGpMA = round(total_xG_ahead/min_ahead,4),
         xGpMA_wt = round(weighted_xG_ahead/min_ahead,4),
         ...
         xG_per_one_ahead = round(total_xG_one_ahead/one_ahead,4),
         xGA_per_one_ahead = round(total_xGA_one_ahead/one_ahead,4)) %>% 
  left_join(league_tables_summary, by = "team") %>% 
  distinct() %>% 
  #normalise values per average minutes ahead per match
  mutate(min_ahead_per_game = round(min_ahead/M,1),
         norm_xGpMA = round(total_xG_ahead/min_ahead_per_game,4),
...
pma_xG_xGA_team <- pma_by_team %>% ungroup() %>% select(team,M,W,
                                                          total_xG_ahead,
                                                          total_xGA_ahead,
                                                          weighted_xG_ahead,
                                                          weighted_xGA_ahead,
